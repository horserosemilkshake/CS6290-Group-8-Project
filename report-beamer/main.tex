...
\begin{itemize}
  ...
  \item Deterministic mock mode (fixed prices, fixed quotes) ensures full reproducibility.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item All four attack vectors: \textbf{0 ALLOW} across 100 adversarial cases.
...
\begin{itemize}
  \item Fixed 125-case dataset; benign set is only 25 cases (FP=0\% confidence interval is wide).
  \item Main benchmark uses deterministic mock quotes, not live DEX API responses.
  \item Only single-turn interactions evaluated; multi-turn attacks not yet covered.
  \item Wallet bridge is in-memory only (demo-ready, not production SDK).
  \item Scope limited to Ethereum mainnet and Sepolia testnet.
\end{itemize}
\end{frame}

\begin{frame}{Future Work}
\begin{itemize}
  \item \textbf{Semantic Intent Verification}: move beyond pattern matching to detect adversarial intent expressed through structurally valid parameters.
  \item \textbf{Multi-turn Attack Evaluation}: assess robustness under extended conversation context poisoning across multiple exchanges.
  \item \textbf{Production Wallet Integration}: bridge to MetaMask / WalletConnect SDK for real owner-action signing flow.
\end{itemize}
\vspace{0.5em}
