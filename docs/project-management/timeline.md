...
| **E - Red Team/Measurement** | • Attack sample generator v0<br>• Attack taxonomy (4 categories: direct injection, indirect/encoded injection, tool-poisoning, memory-poisoning)<br>• Experiment harness skeleton | • attacks/ directory with ≥10 samples per category<br>• run_experiments.py initial version<br>• Sample classification schema | • Manual review: validate attack labels are correct<br>• Reproducibility: same seed produces same samples |
